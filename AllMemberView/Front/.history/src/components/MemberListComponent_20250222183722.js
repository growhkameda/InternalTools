...
// 指定入社月の社員をフィルタリングして表示する
// getEmployeesByJoinMonthとあるが試験的にbirthPlace(出身地)でフィルタリングする
const getEmployeesByJoinMonth = async (birthPlace) => {   //DB実装後引数をjoining_monthに変更
  ...
  if (envType === "stg") {
    getMemberUrl =
      "http://" + process.env.REACT_APP_MY_IP + "/api/users-by-newEmployee?birthPlace=" + birthPlace; //DB実装後引数をjoining_monthに変更
  } else {
    getMemberUrl =
      "http://localhost:8080/allmemberview/api/users-by-newEmployee?birthPlace=" + birthPlace;        //DB実装後引数をjoining_monthに変更
  }
...
const PeopleList = () => {
  ...
  //入社月フィルタ用のステート(試験的にbirthPlace DBA完成後joining_monthに変更)
  const [birtPlace, setBirthPlace] = useState()
