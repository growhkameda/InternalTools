...
*/
const PeopleList = ({fetchMode = "joinMonth"}) => {   //fetchModeのjoinMonthを初期値として設定
  const [userId, setUserId] = useState()              //入社月フィルタ用のステート(DB実装後引数をjoiningMonth(おそらくこの命名)に変更)  ここ初期化する
...
//fetchDataを改修したためコメントアウト(念のために残す)
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      //fetchModeがallだった場合resultにalluserInfoを格納、joinMonthの場合
      if (fetchMode === 'all') {
        ...
        //userIdが定義されている場合getEmployeesByJoinMonthを呼び出す
        if (userId !== undefined) {
