...
const PeopleList = ({fetchMode = "joinMonth", userId: propUserId}) => {
  const [userId, setUserId] = useState(               //fecthModeが"joinMonth"の場合、propUserId(無ければ1)を設定, "all"の場合userIdを使用せずnullに設定
    fetchMode === "joinMonth" ? (propUserId !== undefined ? propUserId : 9) : null   //初期値はjoinMonthに変更時検討する
  );    
  ...
  return (
    ...
    >
      ...
      <Box
        sx={{
          display: "grid",
          gridTemplateColumns: isMobile ? "repeat(2, 200)" : "repeat(4, 200)", // モバイル時は2列、PC時は4列
          gap: 2,
