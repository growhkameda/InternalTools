...
const getEmployeesByJoinMonth = async (birthPlace) => {
  ...
  if (envType === "stg") {
    getMemberUrl =
      "http://" + process.env.REACT_APP_MY_IP + "/api/users-by-newEmployee?birthPlace=" + birthPlace; //渡すパラメータは変更予定
  } else {
    getMemberUrl =
  ...
  try {
    ...
    //dateプロパティがある場合、その値をresponseDateに格納
    if (response.data) {
      responseData = response.data;
    }
  } catch (err) {
