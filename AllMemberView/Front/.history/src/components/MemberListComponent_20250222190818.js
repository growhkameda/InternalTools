...
const getEmployeesByJoinMonth = async (userId) => {   //DB実装後引数をjoiningMonth(おそらくこの命名)に変更
  ...
  if (envType === "stg") {
    getMemberUrl =
      "http://" + process.env.REACT_APP_MY_IP + "/api/users-by-newEmployee?birthPlace=" + birthPlace; //DB実装後引数をjoiningMonth(おそらくこの命名)に変更
  } else {
    getMemberUrl =
      "http://localhost:8080/allmemberview/api/users-by-newEmployee?birthPlace=" + birthPlace;        //DB実装後引数をjoiningMonth(おそらくこの命名)に変更
  }
