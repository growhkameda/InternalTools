...
const PeopleList = ({fetchMode = "joinMonth", userId: propUserId}) => {
  const [userId, setUserId] = useState(               //fecthModeが"joinMonth"の場合、propUserId(無ければ1)を設定, "all"の場合userIdを使用せずnullに設定
    fetchMode === "joinMonth" ? (propUserId !== undefined ? propUserId : 9) : null
  );    
    ...
    >
      ...
      <Box
        sx={{
          ...
          gridTemplateColumns: isMobile ? "repeat(2, 800px)" : "repeat(4, 100px)", // モバイル時は2列、PC時は4列
          gap: 2,
        }}
      >
