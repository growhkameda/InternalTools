...
const getEmployeesByJoiningMonth = async (joiningMonth) => {   //DB実装後引数をJoiningMonth(おそらくこの命名)に変更
  ...
  if (envType === "stg") {
    getMemberUrl =
      "http://" + process.env.REACT_APP_MY_IP + "/api/users-by-newEmployee?joiningMonth=" + joiningMonth; //DB実装後引数をJoiningMonth(おそらくこの命名)に変更
  } else {
    getMemberUrl =
      "http://localhost:8080/allmemberview/api/users-by-newEmployee?joiningMonth=" + joiningMonth;        //DB実装後引数をJoiningMonth(おそらくこの命名)に変更
  }
...
/*
  alluserInfoとgetEmployeesByJoiningMonthを切り替えられるようにfetchModeを作成(初期値はJoiningMonth)
  fetchModeのステートは、  
    "all"       : 全社員を取得するalluserInfoを使用
    "JoiningMonth" : 指定した入社年月を取得するgetEmployeesByJoiningMonthを使用
*/
const PeopleList = ({fetchMode = "JoiningMonth", userId: propUserId}) => {
  const [userId, setUserId] = useState(               //fecthModeが"JoiningMonth"の場合、propUserId(無ければ1)を設定, "all"の場合userIdを使用せずnullに設定
    fetchMode === "JoiningMonth" ? (propUserId !== undefined ? propUserId : 1) : null   //初期値はJoiningMonthに変更時検討する
  );    
  ...
  useEffect(() => {
    const fetchData = async () => {
      let result;
      //fetchModeがallだった場合resultにalluserInfoを格納、JoiningMonthの場合getEmployeesByJoiningMonthを格納
      if (fetchMode === 'all') {
        result = await alluserInfo();
      } else if (fetchMode === 'JoiningMonth') {
        if (userId !== undefined) {
          result = await getEmployeesByJoiningMonth(userId)  //DB実装後引数をJoiningMonth(おそらくこの命名)に変更
        }
      }
      setPeople(result);
    };
