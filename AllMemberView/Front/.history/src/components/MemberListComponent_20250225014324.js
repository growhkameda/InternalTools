...
const getEmployeesByJoinMonth = async (userId) => {   //DB実装後引数をjoiningMonth(おそらくこの命名)に変更
  ...
  if (envType === "stg") {
    getMemberUrl =
      "http://" + process.env.REACT_APP_MY_IP + "/api/users-by-newEmployee?birthPlace=" + userId; //DB実装後引数をjoiningMonth(おそらくこの命名)に変更
  } else {
    getMemberUrl =
      "http://localhost:8080/allmemberview/api/users-by-newEmployee?birthPlace=" + userId;        //DB実装後引数をjoiningMonth(おそらくこの命名)に変更
  }
...
/*
  alluserInfoとgetEmployeesByJoinMonthを切り替えられるようにfetchModeを作成
  fetchModeのステートは、  
    "all"       : 全社員を取得するalluserInfoを使用(初期値として設定)
    "joinMonth" : 指定した入社年月を取得するgetEmployeesByJoinMonthを使用(現在はuserIdを取得)
*/
const PeopleList = ({fetchMode = 'all'}) => {
  ...
  const [userId, setuserId] = useState("") //入社月フィルタ用のステート(DB実装後引数をjoiningMonth(おそらくこの命名)に変更)
  ...
  //alluserInfoとgetEmployeesByJoinMonthを切り替えて呼び出す
  useEffect(() => {
    const fetchData = async () => {
      let result;
      //fetchModeがallだった場合resultにalluserInfoを格納
      if (fetchMode === 'all') {
        result = await alluserInfo();
      } else if (fetchMode === 'joinMonth') {
        result = await getEmployeesByJoinMonth(userId)  
      }
      setPeople(result);
    };
    fetchData();
