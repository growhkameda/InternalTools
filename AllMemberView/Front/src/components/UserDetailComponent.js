...
const UserProfile = () => {
  ...
  useEffect(() => {
    ...
    const fetchUserProfile = async () => {
      try {
        const response = await axios.get(`http://localhost:8080/allmemberview/api/user/${id}`, {
          headers: { Authorization: `Bearer ${token}` },
        });

        setProfile(response.data);
        setImage(`data:image/jpeg;base64,${response.data.user.image}`);
      } catch (err) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      await axios.put(`http://localhost:8080/allmemberview/api/user/${id}`, formData, {
        headers: {
  ...
  const handleDelete = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.delete(`http://localhost:8080/allmemberview/api/user/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  return (
    <Box display="flex" padding={3} sx={{ backgroundImage: 'url(https://image.en-gage.net/image/work_picture/481922/16802414219493benv.jpg?width=573)', backgroundSize: 'cover', backgroundPosition: 'center' }}>
      <Box marginRight={3}>
        <Avatar
          alt={profile.user.userName}
          src={image}
          sx={{ width: 200, height: 200, border: "3px solid #1976d2", boxShadow: 3 }}
        />
        {isEditing && (
